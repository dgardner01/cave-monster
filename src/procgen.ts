...
function* connectRooms(
   ...
   //set the start point of our tunnel at the center of the first room
    ...
    while (current[0] !== end[0] || current[1] !== end[1]) {
        //are we tunneling in positive or negative direction?
        //if direction is - we have hit the destinaton in one direction
        const direction = Math.sign(end[axisIndex] - current[axisIndex]);
        if (direction !== 0) {
            current[axisIndex] += direction;
            yield current;
        } else {
            //we've finished in this direction so switch to other
